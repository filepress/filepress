...
templateCache.set('layout', fs.readFileSync(`./${settings.templatesFolder}/layout.html`))
...
const layouts = () => (page, site) => {
	...
	if(!templateCache.has(page.layout)) {
		...
		const template = fs.readFileSync(path.join(process.cwd(), settings.templatesFolder, `${page.layout}.html`))
