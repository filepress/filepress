...
const filepress = {
	use: (transform) => {
		transformsToUser.push(_.map(obj => transform(obj, settings)))
		return filepress
	},
	observe: (observer) => {
		transformsToUser.push(_.consume((err, x, push, next) => {
			if(err) {
				push(err)
				next()
			} else if(x === _.nil) {
				const observed = observer({last: true}, settings)
				if(observed) push(null, observed)
				push(null, x)
			} else {
				observer(x, settings)
				push(null, x)
...
module.exports.buildIndex = require('./transforms/buildIndex')
module.exports.buildArchive = require('./transforms/buildArchive')
