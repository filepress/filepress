...
const LAST = 'LAST element from filepress.'
...
const filepress = {
    use: transform => {
        transformsToUser.push(_.flatMap(obj => {
            const transformResult = transform(obj, settings)
            if (transformResult.length && transformResult.constructor === Array) {
    ...
    start: sourceFolder => {
        ...
        walk(sourceFolder, (error, file) => {
            files.push(file)
        }, () => {
            const transforms = _.seq(...transformsToUser)
            _(files.concat([LAST]))
                .map(filePath => {
                    if(filePath === LAST) return filePath
                    return {
                        sourcePath: filePath,

                        //Path to file without extension, at first same as link.
                        path: path.relative(sourceFolder, filePath).replace(/\..*$/, ''),
                        link: path.relative(sourceFolder, filePath).replace(/\..*$/, ''),
                        extension: path.parse(filePath).ext
                    }
                })
                .flatMap(file => addMarkdownOld(file))
                .through(transforms)
                .done(() => {})
        })
    },
